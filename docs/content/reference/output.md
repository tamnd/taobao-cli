...
describes what users get. Pick a format with `-o`, or let taobao choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
taobao <command> -o table   # aligned columns for reading
taobao <command> -o jsonl   # one JSON object per line, for piping
taobao <command> -o json    # a single JSON array
taobao <command> -o csv     # spreadsheet friendly
taobao <command> -o tsv     # tab-separated
taobao <command> -o url     # just the URL column
taobao <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
taobao <command> --fields id,title,url
```
...
```bash
taobao <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
taobao <command>            # a table, because this is a terminal
taobao <command> | wc -l    # JSONL, because this is a pipe
```
