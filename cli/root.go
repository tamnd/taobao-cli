// Package cli assembles the taobao command tree from the taobao
...
// NewApp assembles the kit application from the taobao domain. The
...
//
// To add a command, declare it in taobao/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := taobao.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(taobao.Domain{}).Register(app)
